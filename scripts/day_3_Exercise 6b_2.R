...
#### di/dz = epsilon * surface intensity: numerical solution of  ####


df = data.frame(depth1 = vector(), light = vector())
intensity_z = function(depth, surface_intensity, eps, dz = dz){
  y =  surface_intensity
  depth = seq(from = 0, to = depth, by = dz)
  for (i in 2:length(depth)){
    didz = -y[i-1]*eps
    y[i] = y[i-1] + didz*dz
  ...
  return(y)
                  ...
                  #getting the index
which(df$depth %in% 10.5)
match(10.5, df$depth)
  
find_sat_point = function(surface_intensity, half_satu_K, coeff){
  return(log(surface_intensity, exp(1)))
}
  

#P_mean =  Max_Synth_pm*(1/eps)*(eps*surface + 1 - ())
